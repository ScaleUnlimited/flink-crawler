...
public class DomainRankFunction 
	extends BaseCoFlatMapFunction<	Tuple2<String, String>, Tuple2<String, Double>, 
	...
	private class ActiveMassDistribution {
		...
		public ActiveMassDistribution(int sourceDomainIndex) {
			super();
			...
			double massToDistribute = sourceDomainInfo.getMass();
			if (massToDistribute < 0.000001d) {
				logIfDebug(	"%s has no significant mass to distribute", 
							sourceDomain);
			}
			...
			_extraMassPerPartition =
				(	(massToDistribute * _outgoingEpsilonMassFraction)
				/ 	numPartitions);
		...
		int getPartition(String targetDomain) {
			try {
				return _partitioner.partition(	_keySelector.getKey(targetDomain),
												getNumPartitions());
			} catch (Exception e) {
				throw new RuntimeException(	(	"Unable to get key for domain " 
											+ 	targetDomain), 
											e);
			}
		}
	...
	private double _outgoingEpsilonMassFraction =
		DEFAULT_OUTGOING_EPSILON_MASS_FRACTION;
	...
	public void flatMap1(	Tuple2<String, String> domainLink,
		...
		if (sourceDomainInfo == null) {
			...
			_sourceDomainList.add(new Tuple2<String, DomainInfo>(sourceDomain, sourceDomainInfo));
		} else {
	...
	private void distributeMassIfPossible(Collector<Tuple2<String, Double>> collector) {
		...
		if (_activeDistribution == null) {
			...
			_activeDistribution = new ActiveMassDistribution(_sourceDomainIndex);
		}
	...
	private static void logIfDebug(String format, int partition, Object... args) {
		if (LOGGER.isDebugEnabled()) {
			...
			LOGGER.debug(String.format(	"[Partition %d] " + format, 
										fullArgs));
		}
	}
}
