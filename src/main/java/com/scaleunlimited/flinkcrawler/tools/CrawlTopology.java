 ...
 * 
 * Flink currently only provides processing guarantees for jobs without iterations. Enabling
 * checkpointing on an iterative job causes an exception. In order to force checkpointing on an
 * iterative program the user needs to set a special flag when enabling checkpointing:
...
public class CrawlTopology {
    ...
    public static class CrawlTopologyBuilder {
        ...
        private BaseHttpFetcherBuilder _pageFetcherBuilder = new SimpleHttpFetcherBuilder(
                INVALID_USER_AGENT);
        private BaseHttpFetcherBuilder _siteMapFetcherBuilder = new SimpleHttpFetcherBuilder(
                INVALID_USER_AGENT);
        ...
        private SplitStream<Tuple2<CrawlStateUrl, FetchedUrl>> splitFetchedUrlsStream(
            ...
            SplitStream<Tuple2<CrawlStateUrl, FetchedUrl>> fetchAttemptedUrls = fetchedUrls
                    .split(new OutputSelector<Tuple2<CrawlStateUrl, FetchedUrl>>() {
                        private final List<String> FETCH_STATUS_STREAM = Arrays
                                .asList("fetch_status");
                        private final List<String> FETCHED_URL_STREAMS = Arrays.asList(
                                "fetch_status", "fetched_url");
        ...
        private SplitStream<Tuple3<ExtractedUrl, ParsedUrl, String>> splitOutlinkContent(
            ...
            SplitStream<Tuple3<ExtractedUrl, ParsedUrl, String>> outlinksOrContent = parsedUrls
                    .split(new OutputSelector<Tuple3<ExtractedUrl, ParsedUrl, String>>() {
                        ...
                        private final List<String> CONTENT_TEXT_STREAM = Arrays
                                .asList("content_text");
    ...
    /**
     * Trigger async execution, and then monitor the job. Fail if it the job is still running after
