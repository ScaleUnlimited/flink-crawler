...
/**
 * Map from (reversed) URL to information about the primary index file (and offset/length of segment in that file) where
 * the URL would have to be found.
 *
 */
...
public class SecondaryIndexMap {
    ...
    public SecondaryIndex get(String key) {
        ...
        if (index < 0) {
            ...
            // LOGGER.debug(String.format("'%s' should be in segment %d based on secondary value '%s'", key, index,
            // _secondaryIndexUrls[index]));
            // LOGGER.debug(String.format("First url in segment %d is '%s'", index + 1, _secondaryIndexUrls[index +
            // 1]));
        }
    ...
    public static class Builder {
        ...
        private static final Pattern IDX_LINE_PATTERN = Pattern
                .compile("(.+?)[ ]+(\\d+)\t(.+?)\t(\\d+)\t(\\d+)\t(\\d+)");
